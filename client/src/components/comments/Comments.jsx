...
function Comments() {
    ...
    function delteComment(dataId) {
        let filtered
        fetch(`http://localhost:8081/comment/${dataId}`, {
            method: "DELETE",
        })
            .then(response => response.json())
                ...
                return (
                    <div key={i}>
                        <form onSubmit={() => updateComment(event, comment.id, i)}>
                            ...
                            {indexOfComment == i && <button type='submit'>update</button>} </form>
                        {comment.email == user.email && <button disabled={indexOfComment == i} onClick={()=>delteComment(comment.id)} ><AiFillDelete /></button>}
